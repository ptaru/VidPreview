...
class BookmarkManager {
  ...
  func saveBookmark(for url: URL) {
    do {
      ...
      logger.error("Failed to save bookmark for \(url.lastPathComponent): \(error.localizedDescription)")
    }
  }
  ...
  func resolveBookmark(for url: URL) -> URL? {
    ...
    do {
      ...
      logger.error("Failed to resolve bookmark for \(url.lastPathComponent): \(error.localizedDescription)")
      return nil
    }
  }
}
